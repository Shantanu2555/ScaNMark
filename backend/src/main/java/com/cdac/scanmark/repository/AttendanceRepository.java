...
public interface AttendanceRepository extends JpaRepository<Attendance, Long> {
        ...
        @Query("SELECT CASE WHEN COUNT(a) > 0 THEN true ELSE false END FROM Attendance a WHERE a.studentPrn = :prn AND a.lectureDate = :today AND a.isPresent = true")
        Boolean isStudentPresentToday(@Param("prn") Long prn, @Param("today") LocalDate today);

        // Total lectures the student should have attended
        @Query("SELECT COUNT(DISTINCT a.lectureId) FROM Attendance a WHERE a.studentPrn = :prn")
        int countTotalLectures(@Param("prn") Long prn);

        // Total lectures attended by the student
