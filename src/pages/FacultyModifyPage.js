...
function FacultyModifyPage() {
  ...
  const fetchFaculties = async () => {
    try {
      const response = await axios.get("/api/faculty/get-all-faculties", {
        headers: {
  ...
  const handleSaveFaculty = async () => {
    try {
      ...
      await axios.put(
        `/api/coordinators/update-faculty/${editingFacultyCode}`,
  ...
  const handleDeleteFaculty = async (facultyCode) => {
    try {
      await axios.delete(`/api/coordinators/delete-faculty/${facultyCode}`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
      });
